...
/**
 * 客户端"新建/继续问卷"的统一入口（按具体某一份问卷模板）：
 * 1. 该模板下有未提交草稿 -> 直接续填，不受冷却期限制。
 * 2. 无草稿但最近一次提交在冷却期内 -> 拒绝新建。
 * 3. 否则新建一份绑定到该客户账号 + 该模板的问卷。
 */
export async function resolveOrCreateCheckupForClient(
  clientId: string,
  templateId: string
): Promise<CheckupAccessResult> {
  const { prisma } = await import("@/lib/prisma");

  const draft = await prisma.checkup.findFirst({
    where: { clientId, templateId, status: "draft" },
    orderBy: { updatedAt: "desc" },
  });
  if (draft) return { kind: "ok", token: draft.token };

  const lastSubmitted = await prisma.checkup.findFirst({
    where: { clientId, templateId, status: "submitted" },
  ...
  const created = await prisma.checkup.create({
    data: { token, status: "draft", companyName: "", answersJson: {}, clientId, templateId },
  });
