...
const GUIDE_SECTIONS: Array<{ heading: string; items: GuideItem[] }> = [
  {
    ...
    items: [
      ...
      {
        title: "AI配置",
        body: "配置报告生成用到的提示词，共五个标签：拼装模式、融合模式、高级模式各自的效果文案，三方报告提取的提取要求，以及报告末尾的免责声明文案。每份内容按问卷单独保存生效，默认版只读，如需修改先复制一份再编辑。拼装/融合/三方报告提取灰色框内的「系统固定要求」是硬性约束，不可修改；高级模式没有这个限制，整段内容都可以自由编辑。",
      },
    ],
  },
  {
