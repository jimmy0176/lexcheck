...
# 服务器上请复制到 /opt/lexcheck/deploy.sh 再执行，切勿直接执行本仓库内的路径：
#   sudo cp /path/to/deploy-server.sh /opt/lexcheck/deploy.sh && sudo bash /opt/lexcheck/deploy.sh
# 若从 /opt/lexcheck/web/deploy.sh 运行，脚本会 rm -rf web，正在执行的文件被删，Bash 会中途退出。
...
if [[ "$SCRIPT_DIR" == "$APP_DIR" ]]; then
  echo "ERROR: 不能从 $APP_DIR 内执行 deploy（部署会清空该目录）。"
  echo "请执行: sudo cp <仓库里的 deploy-server.sh 或本文件> $BASE_DIR/deploy.sh && sudo bash $BASE_DIR/deploy.sh"
  exit 1
fi
