...
**登录 UI：** `components/auth/EmailLoginForm.tsx`（新，主入口，验证码/密码可切换）+ `components/auth/PhoneCodeLoginForm.tsx`（旧组件不变，作为"手机号"次级标签）。`LoginDialog.tsx`、`/login`、`ClientAuthGate.tsx` 三处入口均已改为"邮箱/手机号"双标签，默认邮箱。
