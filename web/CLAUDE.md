...
**登录 UI：** `components/auth/LoginForm.tsx`——单一账号输入框（不再分邮箱/手机号标签），根据输入内容自动识别类型（含 `@` 判为邮箱，匹配手机号正则判为手机号）决定登录方式：邮箱可选验证码/密码，手机号固定验证码。注册页固定"邮箱必填 + 手机号选填"两个字段（不随登录框的识别结果变化），统一走 `method: "email_code"`。`LoginDialog.tsx`、`/login`、`ClientAuthGate.tsx` 三处入口都直接渲染这一个组件。
